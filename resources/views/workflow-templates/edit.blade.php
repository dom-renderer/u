...
<div class="p-4 rounded">
  <form action="{{ route('workflow-templates.update', $template->id) }}" method="post" id="templateForm">
    @csrf
    @method('PUT')
                    ...
                    <div class="section-item" data-section-id="{{ $section['id'] }}">
                      <div class="d-flex align-items-center p-3">
                        ...
                        <div class="flex-grow-1">
                          ...
                          <div class="text-muted small section-description">{{ $section['description'] ?? 'No description provided' }}</div>
        ...
        <div class="col-md-8">
          <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
              ...
              <div>
                <button type="button" class="btn btn-outline-secondary btn-sm me-2" id="cloneSection" disabled>
...
$(document).ready(function () {
    ...
    function initializeStepEventHandlers(sectionId) {
        ...
        $(document).on('change', '.dependency-select', function() {
            const container = $(this).closest('.step-card').find('.dependency-steps-container');
    ...
    $('#templateForm').on('submit', function(e) {
        ...
        if (validateForm()) {
            $.ajax({
                url: "{{ route('workflow-templates.update', $template->id) }}",
                type: 'POST',
                data: {
                    _token : "{{ csrf_token() }}",
                    data : sections,
