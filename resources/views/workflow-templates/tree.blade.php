...
<div class="p-4">
    ...
    <div class="tree-header d-flex justify-content-between align-items-center">
        <div>
            <div class="tree-header-title">{{ $template->title }}</div>
            <div class="tree-header-subtitle">
                Workflow Tree Visualization • {{ $template->children->count() }} steps
            </div>
        </div>
        <div class="tree-actions">
            <a href="{{ route('workflow-templates.show', encrypt($template->id)) }}" class="btn-back">
...
$(document).ready(function() {
    const templateId = '{{ encrypt($template->id) }}';
    const dataUrl = '{{ route("workflow-templates.tree-data", encrypt($template->id)) }}';
