...
@section('content')
    <div class="p-4 rounded">
        <form action="{{ route('workflow-assignments.update', $assignment->id) }}" method="post" id="templateForm">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
                <div class="col-md-12">
                    ...
                    <select name="new_workflow_template_id" id="workflow_template_id" class="form-control" disabled readonly>
                        @if($assignment->template)
                            <option value="{{ $assignment->template->id }}" selected>{{ $assignment->template->title }}</option>
                        @else
                            <option value="">No Template Selected</option>
                        @endif
                    </select>
                    <input type="hidden" name="new_workflow_template_id" value="{{ $assignment->new_workflow_template_id }}">
                </div>

                <div class="col-md-12">
                    <label class="form-label">Start From <span class="text-danger"> * </span> </label>
                    <input type="datetime-local" name="start_from" id="start_from" class="form-control" 
                        value="{{ old('start_from', $assignment->start_from ? $assignment->start_from->format('Y-m-d\TH:i') : '') }}" readonly>
                </div>

                <div class="col-md-12">
                    <label class="form-label">Title <span class="text-danger"> * </span> </label>
                    <input type="text" name="title" class="form-control" value="{{ old('title', $assignment->title) }}"
                        required>
                </div>
                ...
                <div class="col-md-12">
                    ...
                    <select name="status" id="status" class="form-control">
                        <option value="1" @if($assignment->status == 1) selected @endif> Active </option>
                        <option value="0" @if($assignment->status != 1) selected @endif> InActive </option>
                    </select>
                </div>
            </div>
                                            ...
                                            <div class="d-flex align-items-center p-3">
                                                ...
                                                <div class="flex-grow-1">
                                                    ...
                                                    <div class="d-flex justify-content-between align-items-center mt-1">
                                                        <span class="section-steps-count">{{ $sectionSteps->count() }} steps</span>
...
@push('js')
    ...
    <script>
        $(document).ready(function () {
            let sectionIndex = {{ count($assignment->sections ?? []) }};
            let currentSectionId = null;
            let sections = {!! json_encode($assignment->sections ?? []) !!};
            let globalStepCounter = {{ $assignment->children->max('step') ?? 0 }};
            ...
            @if($assignment->children && count($assignment->children) > 0)
            ...
            $('#templateForm').on('submit', function (e) {
                ...
                if (validateForm()) {
                    $.ajax({
                        url: "{{ route('workflow-assignments.update', $assignment->id) }}",
                        type: 'POST',
                        data: {
                            _token: "{{ csrf_token() }}",
                            data: sections,
