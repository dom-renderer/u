...
@section('content')
    <div class="p-4 rounded">
        <form action="{{ route('workflow-assignments.store') }}" method="post" id="templateForm">
            @csrf
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-12">
                    <label class="form-label">Workflow Template <span class="text-danger"> * </span> </label>
                    <select name="new_workflow_template_id" id="workflow_template_id" class="form-control select2" required>
                        <option value="">Select a Template</option>
                        @foreach($templates as $template)
                            <option value="{{ $template->id }}" {{ old('new_workflow_template_id') == $template->id ? 'selected' : '' }}>{{ $template->title }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-12">
                    ...
                    <input type="datetime-local" name="start_from" id="start_from" class="form-control" value="{{ old('start_from') }}" required>
                </div>
...
@push('js')
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#workflow_template_id').on('change', function() {
                const templateId = $(this).val();
                if (templateId) {
                    $.ajax({
                        url: "{{ route('workflow-assignments.load-template', ':id') }}".replace(':id', templateId),
                        type: 'GET',
                        beforeSend: function() {
                        ...
                        success: function(response) {
                            ...
                            $('#sectionDetailsContainer').html('<div class="text-center py-5"><i class="bi bi-list-ol text-muted" style="font-size: 3rem;"></i><p class="text-muted mt-3">Select a section to configure its details</p></div>');
                            ...
                            // Load sections and steps
                            if (response.sections) {
                                Object.keys(response.sections).forEach(function(sectionId) {
                                    const sectionData = response.sections[sectionId];
                                    sectionIndex++;
                                        ...
                                        <div class="section-item" data-section-id="${sectionId}">
                                            <div class="d-flex align-items-center p-3">
                                                ...
                                                <div class="flex-grow-1">
                                                    <div class="fw-semibold section-name">${sectionData.name || 'New Section'}</div>
                                                    <div class="section-code text-muted">${sectionData.code || 'NEW_SECTION'}</div>
                                                    <div class="text-muted small section-description">${sectionData.description || 'No description provided'}</div>
                                                    <div class="d-flex justify-content-between align-items-center mt-1">
                                                        <span class="section-steps-count">${sectionData.steps ? Object.keys(sectionData.steps).length : 0} steps</span>
                                                        <span class="section-number">Section ${sectionIndex}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    if (sectionData.steps) {
                                        Object.keys(sectionData.steps).forEach(function(stepId) {
                                            const step = sectionData.steps[stepId];
                                            if (step.globalNumber > globalStepCounter) {
                                                globalStepCounter = step.globalNumber;
                                            }
                                        });
                                    }
                                });
            ...
            $('#templateForm').on('submit', function (e) {
                ...
                if (validateForm()) {
                    $.ajax({
                        url: "{{ route('workflow-assignments.store') }}",
                        type: 'POST',
                        data: {
                            _token: "{{ csrf_token() }}",
                            data: sections,
                            new_workflow_template_id: $('#workflow_template_id').val(),
                        ...
                        success: function (response) {
                            Swal.fire('Success', 'Workflow assignment saved succesfully', 'success');
                            location.href = "{{ route('workflow-assignments.index') }}";
                        },
